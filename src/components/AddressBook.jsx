...
const AddressBook = () => {
    ...
    const fetchAddresses = async () => {
        try {
            const res = await fetch(`process.env.REACT_APP_API_URL || '(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')'/addresses/user/${user.userId}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch('process.env.REACT_APP_API_URL || '(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')'/addresses/add', {
                method: 'POST',
    ...
    const deleteAddress = async (id) => {
        try {
            await fetch(`process.env.REACT_APP_API_URL || '(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')'/addresses/${id}`, { method: 'DELETE' });
            fetchAddresses();
        } catch (error) {
            console.log('Error:', error);
        }
    ...
    const setDefault = async (id) => {
        try {
            await fetch(`process.env.REACT_APP_API_URL || '(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')'/addresses/${id}/default`, { method: 'PUT' });
            fetchAddresses();
        } catch (error) {
            console.log('Error:', error);
        }
