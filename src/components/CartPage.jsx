...
const CartPage = () => {
    ...
    const updateQuantity = async (productId, newQty) => {
        ...
        try {
            await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/cart/${productId}?quantity=${newQty}`, { method: 'PUT' });
    ...
    const removeItem = async (productId) => {
        ...
        try {
            await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/cart/${productId}`, { method: 'DELETE' });
    ...
    const applyCoupon = async () => {
        ...
        try {
            const res = await fetch(${process.env.REACT_APP_API_URL || 'http://localhost:8080/api'}/coupons/validate', {
                method: 'POST',
