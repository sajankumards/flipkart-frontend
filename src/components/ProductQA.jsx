...
const ProductQA = ({ productId }) => {
    ...
    const fetchQA = async () => {
        try {
            const res = await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/qa/product/${productId}`);
    ...
    const handleAsk = async () => {
        ...
        try {
            const res = await fetch(${process.env.REACT_APP_API_URL || 'http://localhost:8080/api'}/qa/ask', {
                method: 'POST',
    ...
    const handleVote = async (qaId) => {
        try {
            await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/qa/${qaId}/vote`, { method: 'PUT' });
            fetchQA();
        } catch (error) {
