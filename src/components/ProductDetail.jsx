...
const ProductDetail = () => {
    ...
    const fetchProduct = async () => {
        try {
            const res = await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/products/${id}`);
            if (!res.ok) throw new Error('Not found');
            const data = await res.json();
            setProduct(data);

            const allRes = await fetch(${process.env.REACT_APP_API_URL || 'http://localhost:8080/api'}/products');
    ...
    const fetchReviews = async () => {
        try {
            const res = await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/reviews/product/${id}`);
            const data = await res.json();
            if (data.success) {
    ...
    const checkWishlist = async () => {
        try {
            const res = await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/wishlist/check?userId=${user.userId}&productId=${id}`);
            const data = await res.json();
    ...
    const toggleWishlist = async () => {
        ...
        try {
            if (inWishlist) {
                await fetch(`(process.env.REACT_APP_API_URL || 'http://localhost:8080/api')/wishlist/remove?userId=${user.userId}&productId=${product.id}`, { method: 'DELETE' });
                setInWishlist(false);
                showToast('Removed from Wishlist!', 'info');
            } else {
                await fetch(${process.env.REACT_APP_API_URL || 'http://localhost:8080/api'}/wishlist/add', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const handleReviewSubmit = async (e) => {
        ...
        try {
            const res = await fetch(${process.env.REACT_APP_API_URL || 'http://localhost:8080/api'}/reviews/add', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
